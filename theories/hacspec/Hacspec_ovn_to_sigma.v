...
Module HacspecGroup (OVN_impl : Hacspec_ovn.HacspecOVNParams) (GOP : GroupOperationProperties OVN_impl).
  ...
  Definition zq_ring_theory q :
    ring_theory (R := 'Z_q) (0)%R (1)%R
      (GRing.add) (GRing.mul)
      (fun x y => GRing.add x (GRing.opp y))
      (GRing.opp) eq_op.
  Proof.
    apply (mk_rt (R := 'Z_q) (0)%R (1)%R
      (GRing.add) (GRing.mul)
      (fun x y => GRing.add x (GRing.opp y))
      (GRing.opp) eq_op).
    all: intros.
  ...
  Lemma all_unit_in_prime_field q `{prime q} : forall (p : 'Z_q), (p != 0%R)%N -> p \is a GRing.unit.
  Proof.
    intros.
    assert (1 < q)%N.
    {
    ...
    epose (unitZpE (p := q) p H0).
        setoid_rewrite Zp_nat in e.
        rewrite valZpK in e.

        rewrite e.
        - rewrite (prime_coprime _ prime0).
          cbn.
          rewrite modn_small.
          + now destruct p, m.
          +  destruct p.
             ...
             pose Zp_cast.
             clear -i H0.
             now rewrite Zp_cast in i.
  Qed.
  
  Definition zq_field_theory q `{prime q} :
    field_theory (R := 'Z_q) 0%R 1%R (GRing.add)
  ...
  Proof.
    ...
    }
    {
      ...
      apply (ssrbool.introT eqP).
      apply mulVr.
      now apply all_unit_in_prime_field.
    }
  Defined.

  Lemma zq_ring_eq_ext q : Ring_theory.ring_eq_ext
       (R := 'Z_q) (GRing.add)
       (GRing.mul)
       (GRing.opp) eq_op.
  Proof.
...
Module HacspecGroupParam (OVN_impl : Hacspec_ovn.HacspecOVNParams) (GOP : GroupOperationProperties OVN_impl) (SG : SecureGroup OVN_impl GOP) <: GroupParam.
  ...
  Definition hacspec_zq_ring_theory := @zq_ring_theory q.
  Definition hacspec_zq_field_theory := @zq_field_theory q prime_order.

  Definition hacspec_zq_setoid_structure := (setoid_structure ('Z_q)).
  Definition hacspec_zq_ring_eq_ext := zq_ring_eq_ext q.
...
Module Type OVN_or_proof_preconditions (OVN_impl : Hacspec_ovn.HacspecOVNParams) (GOP : GroupOperationProperties OVN_impl) (SG : SecureGroup OVN_impl GOP).
  ...
  Module MyAlg <: SigmaProtocolAlgorithms MyParam.
    ...
    Definition Extractor (xhy : choiceStatement) (a : choiceMessage)
      ...
      (* refine (let vi := _ in _). *)
      refine (let xi := ((r1' - r1) + (r2' - r2)) / ((d1 - d1') + (d2 - d2')) in _).
      refine (xi, false).
    Defined.
...
Module OVN_or_proof (OVN_impl : Hacspec_ovn.HacspecOVNParams) (GOP : GroupOperationProperties OVN_impl) (SG : SecureGroup OVN_impl GOP) (proof_args : OVN_or_proof_preconditions OVN_impl GOP SG).
 ...
 Lemma or_run_eq  (pre : precond) :
  ...
  Lemma reverse_opp : (forall (x : gT) (n : 'Z_q), x ^+ ((Zp_trunc q).+2 - n) = x ^+ GRing.opp n).
  Proof. now intros ; rewrite trunc_pow. Qed.
  ...
  Proof.
    clear ; intros.
    rewrite mulrV.
    2: apply all_unit_in_prime_field ; [apply prime_order | assumption ].
    now rewrite expg1.
  Qed.

  (* Lemma expg_div : (forall (x : gT) (y z : 'Z_q), x ^+ y ^- z = x ^+ (y / z)%R). *)
  (* Proof. *)
  (*   intros. *)
  (*   destruct y. *)
  (*   induction m ; simpl (nat_of_ord (Ordinal _)). *)
  (*   - set (_ ^+ _). *)
  (*     rewrite expg_sub. *)
  (*     rewrite expgAC. *)
  (*     rewrite expg0. *)
  (*     unfold "/" ; simpl GRing.Nmodule_isSemiRing.mul. *)
  (*     simpl. *)
  (*     rewrite mul0n. *)
  (*     rewrite modn_small. *)
  (*     2: easy. *)
  ...
  (*     epose (prod_swap_left_iff). *)
  (*     symmetry in i0. *)
  (*     symmetry. *)
  (*     apply i0. *)
  (*     rewrite mulgA. *)
  (*     replace (x ^+ (z^-1)%R) with (x ^- z). *)
  (*     2:{ *)
  (*       epose canRL. *)
  ...
  Proof.
    ...
    (* assert (y ^+ (otf e - otf e')%R = h ^+ ((rr1 - lr1) + (rr2 - lr2))%R * g ^+ (ld2 - rd2)%R). *)
    (* { *)
    (*   rewrite (trunc_pow h). *)
    (*   rewrite expgD. *)
    (*   rewrite H8. *)
    ...
    (*   rewrite neg_is_opp. *)
    (*   rewrite <- !expgD. *)
    (*   replace (g ^+ ((GRing.opp (_ - rd2))%R + (ld2 - rd2)%R)) with *)
    (*     (g ^+ (0 - (ld2 - rd2) + (ld2 - rd2))%R) by now rewrite trunc_pow ; rewrite sub0r. *)
    (*   rewrite subrK. *)
    (*   rewrite expg0. *)
    (*   rewrite mulg1. *)
      
    (*   rewrite H'd2d1 Hd2d1. *)
    (*   replace ((otf _ - ld1 - (otf e' - rd1))%R) with *)
    (*     ((rd1 - ld1) + (otf e - otf e'))%R. *)
    (*   2:{ *)
    (*     rewrite (addrC (otf e) (GRing.opp ld1)). *)
    (*     rewrite <- (addrC (GRing.opp ld1)). *)
    (*     rewrite <- !addrA. *)
    (*     f_equal. *)
    (*     symmetry. *)
    (*     apply /eqP. *)
    ...
    assert (x ^+ ((ld1 - rd1) + (ld2 - rd2))%R = g ^+ ((rr1 - lr1) + (rr2 - lr2))%R).
    {
      rewrite !(trunc_pow).
      rewrite !(expgD).
    ...
    assert (y ^+ ((ld1 - rd1) + (ld2 - rd2))%R (* * g ^- nat_of_ord (ld2 - rd2) *) = h ^+ ((rr1 - lr1)%R + (rr2 - lr2)%R)%R).
    {
      rewrite !(trunc_pow y).
      rewrite !(trunc_pow h).
      rewrite !(expgD h).
