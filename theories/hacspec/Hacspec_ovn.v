...
(* File automatically generated by Hacspec *)
Set Warnings "-notation-overridden,-ambiguous-paths".
From SSProve.Crypt Require Import choice_type Package Prelude jasmin_word.
Import PackageNotation.
From extructures Require Import ord fset.
From mathcomp Require Import word_ssrZ word.
(* From Jasmin Require Import word. *)
...
Equations f_cvp_zkp_random_w (s : both t_CastVoteParam) : both v_Z :=
  f_cvp_zkp_random_w s  :=
    bind_both s (fun x =>
      solve_lift (ret_both (snd (fst (fst (fst x))) : v_Z))) : both v_Z.
Fail Next Obligation.
Equations f_cvp_zkp_random_r (s : both t_CastVoteParam) : both v_Z :=
  f_cvp_zkp_random_r s  :=
    bind_both s (fun x =>
      solve_lift (ret_both (snd (fst (fst x)) : v_Z))) : both v_Z.
Fail Next Obligation.
...
Equations f_rp_zkp_random (s : both t_RegisterParam) : both v_Z :=
  f_rp_zkp_random s  :=
    bind_both s (fun x =>
      solve_lift (ret_both (snd x : v_Z))) : both v_Z.
Fail Next Obligation.
...
Equations schnorr_zkp (random : both f_Z) (h : both v_G) (x : both f_Z) : both (t_SchnorrZKPCommit) :=
  schnorr_zkp random h x  :=
    solve_lift (run (letb r := random in
    letb u := f_g_pow r in
...
Equations tally_votes (_ : both t_TallyParameter) (state : both (t_OvnContractState)) : both (t_Result (v_A × t_OvnContractState) t_ParseError) :=
  tally_votes _ state  :=
    ...
    letb '(curr,tally) := foldi_both_list (f_into_iter (Build_t_Range (f_start := ret_both (0 : int32)) (f_end := cast_int (WS2 := U32 (* actual u32 *)) n))) (fun i =>
      ssp (fun '(curr,tally) =>
