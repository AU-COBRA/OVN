    ...
    fn from(nre: NewContractNameError) -> Self {
        let error_code = match nre {
            ...
            NewContractNameError::ContainsDot => unsafe {
                crate::num::NonZeroI32::new_unchecked(i32::MIN + 6) // TODO
            },
    ...
    fn from(nre: NewReceiveNameError) -> Self {
        let error_code = match nre {
            ...
            NewReceiveNameError::InvalidCharacters => unsafe {
                crate::num::NonZeroI32::new_unchecked(i32::MIN + 8) // TODO?
            }
        };
        Self {
            error_code,
    ...
    fn seek(&mut self, pos: SeekFrom) -> Result<u64, Self::Err> {
        ...
        match pos {
            Start(offset) => match u32::try_from(offset) {
                Ok(offset_u32) => {
                    self.current_contract_state_position = offset_u32;
                    Ok(offset)
                }
                _ => Err(()),
            },
            End(delta) => {
                ...
                if delta >= 0 {
                    match u32::try_from(delta)
                        .ok()
                        .and_then(|x| self.current_contract_state_position.checked_add(x))
                    {
                        Some(offset_u32) => {
                            self.current_contract_state_position = offset_u32;
                    ...
                    match delta.checked_abs().and_then(|x| u32::try_from(x).ok()) {
                        Some(before) if before <= end => {
                            ...
                            self.current_contract_state_position = new_pos;
                            Ok(u64::from(new_pos))
                        }
                        _ => Err(()),
                    }
            ...
            Current(delta) => {
                let new_offset = if delta >= 0 {
                    u32::try_from(delta).ok().and_then(|x| self.current_contract_state_position.checked_add(x))
                } else {
                    delta
                        .checked_abs()
                        .and_then(|x| u32::try_from(x).ok())
                        .and_then(|x| self.current_contract_state_position.checked_sub(x))
                };
                match new_offset {
                    Some(offset) => {
                        self.current_contract_state_position = offset;
                        Ok(u64::from(offset))
                    }
                    _ => Err(()),
                }
...
impl Read for ContractState {
    fn read(&mut self, buf: &mut [u8]) -> ParseResult<usize> {
        ...
        let num_read = unsafe { load_state(buf.as_mut_ptr(), len, self.current_contract_state_position) };
        self.current_contract_state_position += num_read;
    ...
    fn read_u64(&mut self) -> ParseResult<u64> {
        ...
        let num_read =
            unsafe { load_state(bytes.as_mut_ptr() as *mut u8, 8, self.current_contract_state_position) };
        self.current_contract_state_position += num_read;
        if num_read == 8 {
    ...
    fn read_u32(&mut self) -> ParseResult<u32> {
        ...
        let num_read =
            unsafe { load_state(bytes.as_mut_ptr() as *mut u8, 4, self.current_contract_state_position) };
        self.current_contract_state_position += num_read;
        if num_read == 4 {
    ...
    fn read_u8(&mut self) -> ParseResult<u8> {
        ...
        let num_read =
            unsafe { load_state(bytes.as_mut_ptr() as *mut u8, 1, self.current_contract_state_position) };
        self.current_contract_state_position += num_read;
        if num_read == 1 {
...
impl Write for ContractState {
    ...
    fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Err> {
        ...
        if self.current_contract_state_position.checked_add(len).is_none() {
            return Err(());
        }
        let num_bytes = unsafe { write_state(buf.as_ptr() as *mut u8, len, self.current_contract_state_position) };
        self.current_contract_state_position += num_bytes; // safe because of check above that len + pos is small enough
...
impl HasContractState<()> for ContractState {
    ...
    fn open(_: Self::ContractStateData) -> Self {
        ContractState {
            current_contract_state_position: 0,
        }
    }
    ...
    fn truncate(mut self, new_size: u32) -> Self {
        let cur_size = self.size();
        if cur_size > new_size {
            unsafe { resize_state(new_size) };
        }
        if new_size < self.current_contract_state_position {
            self.current_contract_state_position = new_size
        }
        self
    }
}
...
impl Read for Parameter {
    fn read(&mut self, buf: &mut [u8]) -> ParseResult<usize> {
        ...
        let num_read =
            unsafe { get_parameter_section(buf.as_mut_ptr(), len, self.current_parameter_position) };
        self.current_parameter_position += num_read;
        Ok(num_read as usize)
    }
}
...
impl HasPolicy for Policy<AttributesCursor> {
    ...
    fn next_item(mut self, mut buf: [u8; 31]) -> (Option<(AttributeTag, u8)>, [u8; 31], Self) {
        if self.items.remaining_items == 0 {
            return (None, buf, self);
        }
        ...
        let (tag_value_len, num_read) = unsafe {
            ...
            let num_read = get_policy_section(
                ...
                2,
                self.items.current_attribute_cursor_position,
            );
            (tag_value_len.assume_init(), num_read)
        };
        self.items.current_attribute_cursor_position += num_read;
        ...
        (Some((AttributeTag(tag_value_len[0]), tag_value_len[1])), buf, self)
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(Policy {
            ...
            items: AttributesCursor {
                current_attribute_cursor_position: attributes_start,
                remaining_items,
            },
        })
    }
...
impl<T: sealed::ContextType> HasCommonData for ExternContext<T> {
    ...
    fn parameter_cursor(&self) -> Self::ParamType {
        Parameter {
            current_parameter_position: 0,
        }
    }
}

/// # Trait implementations for the init context
impl HasInitContext for ExternContext<crate::concordium_types::InitContextExtern> {
    type InitData = ();
...
impl HasReceiveContext for ExternContext<crate::concordium_types::ReceiveContextExtern> {
    type ReceiveData = ();
...
impl HasLogger for Logger {
    ...
    fn log_raw(self, event: &[u8]) -> (Result<(), LogError>, Self) {
        ...
        match res {
            1 => (Ok(()), self),
            0 => (Err(LogError::Full), self),
            _ => (Err(LogError::Malformed), self),
        }
    }
}
...
impl HasActions for Action {
    ...
    fn and_then(self, then: Self) -> Self {
        let res = unsafe { combine_and(self._private_action, then._private_action) };
        Action {
            _private_action: res,
        }
    }
    ...
    fn or_else(self, el: Self) -> Self {
        let res = unsafe { combine_or(self._private_action, el._private_action) };
        Action {
            _private_action: res,
        }
    }
